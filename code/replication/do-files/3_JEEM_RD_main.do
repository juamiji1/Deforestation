...
*All municipalities 
eststo r1: reghdfe floss_prim_ideam_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2!=., abs(${fes}) vce(robust) keepsing
...
eststo r2: reghdfe floss_prim_ideam_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2==1, abs(${fes}) vce(robust) keepsing

summ floss_prim_ideam_area_v2 if e(sample)==1, d
gl mean_y2=round(r(mean), .01)

*Municipalities under a CAR in which Gobernor is NOT mandated as director 
eststo r3: reghdfe floss_prim_ideam_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2==0, abs(${fes}) vce(robust) keepsing
...
* Illegal
*-------------------------------------------------------------------------------
eststo s1: reghdfe floss_prim_ilegal_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2!=., abs(${fes}) vce(robust) keepsing
summ floss_prim_ilegal_area_v2 if e(sample)==1, d
gl mean_sy1=round(r(mean), .01)

eststo s2: reghdfe floss_prim_ilegal_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2==1, abs(${fes}) vce(robust) keepsing
summ floss_prim_ilegal_area_v2 if e(sample)==1, d
gl mean_sy2=round(r(mean), .01)

eststo s3: reghdfe floss_prim_ilegal_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2==0, abs(${fes}) vce(robust) keepsing
summ floss_prim_ilegal_area_v2 if e(sample)==1, d
gl mean_sy3=round(r(mean), .01)

coefplot (s1, label(All)) (s2, label(Governor is head)) (s3, label(Governor not head)), keep(mayorallied) ///
...
* Legal
*-------------------------------------------------------------------------------
eststo p1: reghdfe floss_prim_legal_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2!=., abs(${fes}) vce(robust) keepsing
summ floss_prim_legal_area_v2 if e(sample)==1, d
gl mean_py1=round(r(mean), .01)

eststo p2: reghdfe floss_prim_legal_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2==1, abs(${fes}) vce(robust) keepsing
summ floss_prim_legal_area_v2 if e(sample)==1, d
gl mean_py2=round(r(mean), .01)

eststo p3: reghdfe floss_prim_legal_area_v2 ${controls} [aw=tweights] ${if} & director_gob_law_v2==0, abs(${fes}) vce(robust) keepsing
summ floss_prim_legal_area_v2 if e(sample)==1, d
gl mean_py3=round(r(mean), .01)

coefplot (p1, label(All)) (p2, label(Governor is head)) (p3, label(Governor not head)), keep(mayorallied) ///
...
*-------------------------------------------------------------------------------
*Exporting results 
esttab r1 r2 r3 s1 s2 s3 p1 p2 p3 using "${tables}/rdd_main_results.tex", keep(mayorallied) ///
se nocons star(* 0.10 ** 0.05 *** 0.01) ///
label nolines fragment nomtitle nonumbers obs nodep collabels(none) booktabs b(3) replace ///
prehead(`"\begin{tabular}{@{}l*{9}{c}}"' ///
            `"\hline \toprule"'                     ///
            `" & \multicolumn{3}{c}{Primary Forest Loss (\%)} & \multicolumn{3}{c}{Illegal Forest Loss (\%)} & \multicolumn{3}{c}{Legal Forest Loss (\%)} \\ \cmidrule(l){2-4} \cmidrule(l){5-7} \cmidrule(l){8-10}"' ///
            `" & All & Governor & Governor & All & Governor & Governor & All & Governor & Governor \\"' ///
            `" &  & is head & not head &  & is head & not head &  & is head & not head \\"' ///
            `" & (1) & (2) & (3) & (4) & (5) & (6) & (7) & (8) & (9) \\"'                       ///
            `" \toprule"')  ///
    postfoot(`" Dependent mean & ${mean_y1} & ${mean_y2} & ${mean_y3} & ${mean_sy1} & ${mean_sy2} & ${mean_sy3} & ${mean_py1} & ${mean_py2} & ${mean_py3} \\\\"' ///
