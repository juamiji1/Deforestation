...
gen ln_votantes_muni=log(votantes_muni)
...
gl geovars "ln_area sh_area pre_sh_area_agro sh_area_forest altura mean_sut_crops ln_dist_mcados pre_bii"
...
foreach yvar of global geovars {
	...
	eststo g`i': reghdfe std_`yvar' ${controls} [aw=tweights] ${if}, abs(${fes}) vce(robust)
...
foreach yvar of global demovars {
	...
	eststo d`i': reghdfe std_`yvar' ${controls} [aw=tweights] ${if}, abs(${fes}) vce(robust)
...
gl econvars "pre_ln_va pre_night_light ln_nl pre_desemp_fisc_index pre_ln_regalias pre_ln_inv_total pre_ln_inv_ambiental pre_sh_invenv pre_sh_area_coca pre_sh_area_bovino pre_floss_prim_ideam_area"
...
foreach yvar of global econvars {
	...
	eststo e`i': reghdfe std_`yvar' ${controls} [aw=tweights] ${if}, abs(${fes}) vce(robust)
