...
* Optimal bandwidth
*
* REPLICATION PACKAGE: added. In the working version of this file ${h} was
* first used on line 10 but not defined until line 22, so it only resolved
* because 2_JEEM_RD_lc_assump.do had run earlier in the same Stata session and
* left the global behind. Deriving it here makes the file runnable on its own.
* Same rdrobust call as below, so the bandwidth is unchanged (0.098).
*-------------------------------------------------------------------------------
gl fes "region year"
