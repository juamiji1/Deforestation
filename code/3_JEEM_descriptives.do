...
gr export "${plots}/desc_OLS_plot.png", as(png) replace
...
addplot(scatteri .14 .55 (3) "Governor as REPA Head" .14 1.05 (3) "Governor not REPA Head", mcolor(white) mlabsize(medsmall))

gr export "${plots}/desc_OLS_plot_bygovhead.png", as(png) replace
...
coefplot (mat(B[1]), label("Politicians minority") mcolor("gs4")), vert noci recast(connected) xline(4, lp(dash)) xline(8, lp(dash)) xline(12, lp(dash)) xline(16, lp(dash)) xline(20, lp(dash))  l2title("Primary Forest Loss (%)", size(medium)) b2title("Years", size(medium)) addplot(scatteri .16 12 .16 13 .16 14 .16 15 .16 16, recast(area) color(gs5%20) lcolor(white) base(0.05)) plotregion(margin(zero))

gr export "${plots}/desc_all_yearly_trend.png", as(png) replace
...
coefplot (mat(b0[1]), label("Politicians minority") mcolor("gs9")) (mat(b1[1]), label("Politicians majority") color("gs4")), vert noci recast(connected) xline(4, lp(dash)) xline(8, lp(dash)) xline(12, lp(dash)) xline(16, lp(dash)) xline(20, lp(dash))  l2title("Primary Forest Loss (%)", size(medsmall)) b2title("Years", size(medsmall)) addplot(scatteri .25 12 .25 13 .25 14 .25 15 .25 16, recast(area) color(gs5%20) lcolor(white) base(0.03)) plotregion(margin(zero))

gr export "${plots}/desc_all_yearly_trend_bygovhead.png", as(png) replace
...
coefplot (mat(bf1[1]), label("Party alignment")), vert yline(0, lp(dash) lc(maroon)) noci recast(connected) xline(4, lp(dash)) l2title("Primary Forest Loss (%)", size(medsmall)) b2title("Relative Time to Treatment", size(small)) 

gr export "${plots}/desc_relativetime_govhead.png", as(png) replace
...
gr export "${plots}/desc_relativetime_govnothead.png", as(png) replace
